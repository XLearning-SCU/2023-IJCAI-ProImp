...
def clustering(x_list, y):
    ...
    y_preds = get_y_preds(y, kmeans_assignments, n_clusters)
    #kmeans
    # kmeans_assignments=kmeans(x_final_concat,n_clusters)
    if np.min(y) == 1:
        y = y - 1
    scores, _ = clustering_metric(y, kmeans_assignments, n_clusters)
...
#     LimitKmeans=True
#     if LimitKmeans:
#         kmeans.max_points_per_centroid = 600
...
#     D, I = index.search(feature_vec, 1) # for each sample, find cluster distance and assignments
